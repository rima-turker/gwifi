...
public class HeuristicBasedOnEntityVector {
	...
	public static String getBestMatchingCategory(String shortText) {
		...
		try {
			...
			for (Category mainCat : setMainCategories) { //iterate over categories and calculate a score for each of them
				List<String> lstEntites = new ArrayList<String>();
				for (Annotation a : filteredAnnotations) {
					if (!AnnonatationUtil.getEntityBlackList_AGNews().contains(a.getId())&&WikipediaSingleton.getInstance().getArticle(a.getTitle())!=null) { //we had so many noisy entities therefore filtering required
						lstEntites.add(String.valueOf(a.getId()));
					} 
				}  
				double score=BasedOnWordsCategorize.getSimilarity(lstEntites, String.valueOf(WikipediaSingleton.getInstance().getArticle(mainCat.getTitle()).getId()));
			...
			return firstElement.getTitle()+"\t\t"+sortedMap.get(firstElement);
			//			return firstElement;
	...
	public static Category getBestMatchingCategory(String shortText, List<Category> gtList) {
		...
		try {
			...
			for (Category mainCat : setMainCategories) { //iterate over categories and calculate a score for each of them
				...
				//				mapScore.put(mainCat, BasedOnWordsCategorize.getSimilarity(lstEntites, String.valueOf(WikipediaSingleton.getInstance().getArticle(mainCat.getTitle()).getId())));
			...
			for (Entry<Category, Double> e : sortedMap.entrySet()) {
				mainBuilder.append(e.getKey().getTitle()+"\t"+e.getValue()+"\n");
			}
			mainBuilder.append(firstElement.getTitle()+"\n");
...
//			}
			...
			//			}
						if (!gtList.contains(firstElement)) {
							...
							secondLOG.info(firstElement+"\t"+sortedMap.get(firstElement)+"\t"+((sortedMap.get(firstElement))-(valuesList.get(index))+"\t"+(sortedMap.get(firstElement))/(valuesList.get(index))));
						}
						else {
							//				resultLog.info(firstElement+"\t"+sortedMap.get(firstElement)/totalScore);
							resultLog.info(firstElement+"\t"+sortedMap.get(firstElement)+"\t"+(sortedMap.get(firstElement)-valuesList.get(index))+"\t"+(sortedMap.get(firstElement))/(valuesList.get(index)));
						}
